# Legacy research_routes.py kept for PostgreSQL migration reference.
...
def admin_update_role():
    ...
    if email == current_email:
        return jsonify({
        "status": "error",
        "error": "You cannot change your own role"}), 400
...
# =========================================================
# ACCOUNTS / AKTYWACJA/DEZAKTYWACJA
# =========================================================
...
def admin_toggle_account_active():
    ...
    if email == current_email and is_active is False:
        return jsonify({
        "status": "error",
...
def upload_csv():
    ...
    temp_directory = "data/uploads/temp"
    ...
    try:
        ...
        if not rows:
            return jsonify({
                "status": "error",
                "error": "CSV parser returned no records",
            }), 400

        # Usuń rekordy całkowicie pozbawione danych pomiarowych.
        valid_rows = [
            row
            for row in rows
            if (
                row.get("spo2") is not None
                or row.get("pulse") is not None
                or row.get("heart_rate") is not None
            )
        ]
        ...
        if not valid_rows:
            return jsonify({
                "status": "error",
                "error": (
                    "CSV contains no valid SpO2 or pulse data. "
                    "Check column mappings for oxygen_level and pulse_rate."
                ),
            }), 400

        print("=" * 50)
        print("CSV PARSED:", len(rows))
        print("CSV VALID:", len(valid_rows))
        print("SESSION:", session_id)
        print("FIRST RECORD:", valid_rows[0])
        print("=" * 50)
 ...
 (
        ...
        return jsonify({
            ...
            "records_saved": len(valid_rows),
            "records_rejected": len(rows) - len(valid_rows),
            "session_id": session_id,
            "user_id": user_id,
            "source": "pulseox",
            "device": "Checkme O2",
            "file": filename,
            "first_timestamp": valid_rows[0].get("timestamp"),
            "last_timestamp": valid_rows[-1].get("timestamp"),
        })
